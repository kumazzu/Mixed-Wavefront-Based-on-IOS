...
def resnet50():
    ...
    pv = placeholder(output_shape=(3, 224, 224))
    v, block1, out_channels = resnet_front(pv)
    v, block2, out_channels = resnet_block(v, block_func=bottleneck, expansion=1, channels=64,  layers=3, in_channels=out_channels, stride=(1, 1))
    v, block3, out_channels = resnet_block(v, block_func=bottleneck, expansion=1, channels=128, layers=4, in_channels=out_channels, stride=(2, 2))
    v, block4, out_channels = resnet_block(v, block_func=bottleneck, expansion=1, channels=256, layers=6, in_channels=out_channels, stride=(2, 2))
    v, block5, out_channels = resnet_block(v, block_func=bottleneck, expansion=1, channels=512, layers=3, in_channels=out_channels, stride=(2, 2))
