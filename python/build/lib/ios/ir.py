...
class Block:
    ...
    def wave_schedule_for_blocks_back_impl(self, max_num_stream):
        selected_nodes = []
        free_nodes = []
        self.stages = []
        ...
        while len(free_nodes) > 0:
            ...
            for node in free_nodes:
                if node in node_need_remove:
                    continue
                # ipdb.set_trace()
                if node is not self.enter_node:
                    chain = []
                    
                    # 节点有多个父节点
                    if len(node.inputs) != 1:    
                        chain.append(node)
                    elif (len(node.inputs) == 1) & (len(node.inputs[0]) > 1):
                        chain.append(node)
                       
                    else:       
                    # 节点仅一个父节点             
                        chain.append(node)
                        next_node = node.inputs[0][0].node
                        while (len(next_node.inputs) == 1) & (len(next_node.inputs[0]) == 1):
                            if (next_node is not self.enter_node) & (len(next_node.inputs[0]) == 1):
                                parent_uses = []
                                flag = 0
                                for i in range(len(next_node.uses)):
                                    parent_uses.append(next_node.uses[i][0])
                                for child in parent_uses:
                                    if (child is not chain[-1]) & (child in free_nodes):
                                        flag = flag + 1
                                if (flag == 0) & (chain[-1] in parent_uses) & (next_node is not self.enter_node.uses[0][0]):
                                    chain.append(next_node)
                                if next_node in free_nodes:
                                    # free_nodes.remove(next_node)
                                    node_need_remove.append(next_node)

                                node = next_node
                                next_node = node.inputs[0][0].node
                            else:
                                break
                        if (len(next_node.uses) == 1) & (next_node is not self.enter_node) & (next_node not in free_nodes):
                            chain.append(next_node)
            ...
            # 防止在free_nodes里加入重复的节点
            for chain in wave:
                last_node = chain[-1]
                for i in range(len(last_node.inputs)):
                    for j in range(len(last_node.inputs[i])):
                        parent_node = last_node.inputs[i][j].node
                        if (parent_node is not self.enter_node) & (parent_node not in free_nodes):
                            free_nodes.append(parent_node)

                # 调整chain中节点顺序
                chain.reverse()                 

            wave_parts = []
            for i in range(0, len(wave), max_num_stream):
                wave_parts.append(wave[i:i + max_num_stream])

            for wave in wave_parts:
                self.stages.append([])
                for chain in wave:
                    # 适配nasnet，nasnet中存在多个深层节点依赖浅层节点的情况：
                    # 为防止重复执行浅层节点需要在后续链加入以后剔除原本self.stages中重复的节点
                    for node in chain:  
                        if node in selected_nodes:
                            for wave_in_stages in self.stages:
                                for chain_in_stages in wave_in_stages:
                                    if (node.name in chain_in_stages):
                                        chain_in_stages.remove(node.name)
                                    if len(chain_in_stages) == 0:
                                            wave_in_stages.remove(chain_in_stages)
                                if (len(wave_in_stages) == 0) & (wave_in_stages is not self.stages[-1]):
                                    self.stages.remove(wave_in_stages)
                                    
                        else:
                            selected_nodes.append(node)

                    self.stages[-1].append([node.name for node in chain])

        self.stages.reverse()

    def wave_schedule_for_block_parts_back_impl(self, id, max_num_stream):
        selected_nodes = []
        free_nodes = []
        # self.stages = []
        block_part = self.parts[id]

        free_nodes.append(block_part[-1])
        ...
        while len(free_nodes) > 0:
            ...
            for node in free_nodes:
                ...
                if node in block_part:
                    ...
                    # 节点仅一个父节点             
                        chain.append(node)
                        next_node = node.inputs[0][0].node
                        while (len(next_node.inputs) == 1) & (len(next_node.inputs[0]) == 1) & (next_node in block_part):
                            parent_uses = []
                            flag = 0
                            for i in range(len(next_node.uses)):
                                parent_uses.append(next_node.uses[i][0])
                            for child in parent_uses:
                                if (child is not chain[-1]) & (child in free_nodes):
                                    flag = flag + 1
                                # if (flag == 0) & (chain[-1] in parent_uses) & (next_node is not self.enter_node.uses[0][0]):
                            if (flag == 0) & (chain[-1] in parent_uses):
                                chain.append(next_node)
                            if next_node in free_nodes:
                                node_need_remove.append(next_node)
                        ...
                        if (len(next_node.uses) == 1) & (next_node in block_part) & (next_node not in free_nodes):
                            chain.append(next_node)
            ...
            for chain in wave:
                ...
                for i in range(len(last_node.inputs)):
                    for j in range(len(last_node.inputs[i])):
                        parent_node = last_node.inputs[i][j].node
                        if (parent_node in block_part) & (parent_node not in free_nodes):
                            free_nodes.append(parent_node)
    ...
    def wave_schedule_forward_impl(self, max_num_stream):
        # wave_schedule前向实现：反向实现与前向实现会产生不同的计算图
        self.stages = []
        free_nodes = []
        selected_nodes = []
        # free_nodes.append(self.enter_node)
        for i in range(len(self.enter_node.uses)):
            free_nodes.append(self.enter_node.uses[i][0])
        
        nodes_in_block = []
        nodes_in_block.append(self.enter_node)
        for node in self.inner_nodes:
            nodes_in_block.append(node)
        nodes_in_block.append(self.exit_node)

        while(len(free_nodes) > 0):
            wave = []

            # 节点只有唯一子节点和唯一父节点就顺着下去组成chain，有多个子节点或多个父节点就加入当前wave
            for node in free_nodes:
                chain = []

                if (node in nodes_in_block):  # 节点不在当前块，先不管
                    chain.append(node)
                    
                if (len(node.inputs) == 1) & (len(node.inputs[0]) == 1) & (len(node.uses) == 1) & (node in nodes_in_block):
                    next_node = node.uses[0][0]
                    while (len(next_node.inputs) == 1) & (len(next_node.inputs[0]) == 1) & (next_node in nodes_in_block):
                        chain.append(next_node)
                        if len(next_node.uses) > 1:
                            break
                        else:
                            node = next_node
                            if len(node.uses) != 0:
            ...
            for chain in wave:
                last_node = chain[-1]
                for i in range(len(last_node.uses)):
                    ### 需要在加入free_nodes前删除重复的节点
                    if last_node.uses[i][0] in free_nodes:
                        free_nodes.remove(last_node.uses[i][0])
                    free_nodes.append(last_node.uses[i][0])
...
class Graph:
    ...
    def wave_schedule_backward_impl(self, max_num_stream=10):
        for block in self.blocks:
            # print(block.parts)
            if block.parts is not None:
                block.stages = []
                block.parts.reverse()
                for i in range(len(block.parts)):
                    block.wave_schedule_for_block_parts_back_impl(i, max_num_stream)
                block.parts.reverse()
                block.stages.reverse()
            else:
                block.wave_schedule_for_blocks_back_impl(max_num_stream)

    def wave_schedule_forward_impl(self, max_num_stream=10):
        for block in self.blocks:
            block.wave_schedule_forward_impl(max_num_stream)

    def wave_schedule_mix(self, batch_size=1, max_num_stream=10, warmup=2, number=6, repeat=20):
        # 对合适的图做merge优化
        # merge_graph = ios.optimize(self, batch_size=batch_size, opt_type="dp_merge")
        # merge_latencys = ios.ios_runtime.graph_latency(merge_graph, batch_size=batch_size, warmup=warmup, number=number, repeat=repeat)
        # merge_latency = np.mean(merge_latencys)

        for block in self.blocks:
            block.wave_schedule_forward_impl(max_num_stream)
            forward_opt_latency = ios.ios_runtime.block_latency(block, batch_size=batch_size, warmup=warmup, number=number, repeat=repeat)
            forward_latency = np.mean(forward_opt_latency)
            
            if block.parts is None:
                block.wave_schedule_for_blocks_back_impl(max_num_stream)
                back_opt_latency = ios.ios_runtime.block_latency(block, batch_size=batch_size, warmup=warmup, number=number, repeat=repeat)
                back_latency = np.mean(back_opt_latency)

                if forward_latency < back_latency:
                    block.wave_schedule_forward_impl(max_num_stream)
                # else:
                #     block.wave_schedule_forward_impl(max_num_stream)
            # else:
        ...
        wave_latencys = ios.ios_runtime.graph_latency(self, batch_size=batch_size, warmup=warmup, number=number, repeat=repeat)
        wave_latency = np.mean(wave_latencys)
                    ...
                    for chain in stage:
                        if len(chain) == 1:
                            for node in all_nodes:
                                if (node.name == chain[0]) & isinstance(node, Conv):
                                    flag_for_merge = flag_for_merge + 1
                if flag_for_merge == len(stage):
                    merge_flag = True

        if merge_flag:
            print('MW merge start')
            merge_graph = ios.optimize(self, batch_size=batch_size, opt_type="dp_parallel_merge")
            merge_latencies = ios.ios_runtime.graph_latency(merge_graph, batch_size=batch_size, warmup=warmup, number=number, repeat=repeat)
            merge_latency = np.mean(merge_latencies)

            if merge_latency < wave_latency:
                self.blocks = merge_graph.blocks

            # 当block分配好全部的stages之后，对个别stage中的算子做merge
            # all_nodes = block.inner_nodes + [block.exit_node]
            # index = 0
            # for stage in block.stages:
            #     if len(stage) > 1:
            ...
            #                 for node in all_nodes:
            #                     if (node.name == chain[0]) & isinstance(node, Conv):
            #                         nodes_for_merge.append(node)
            #                 # if isinstance(chain[0], Conv):
            #                     # nodes_for_merge.append(chain[0])
            #         if len(nodes_for_merge) > 1:
            #             # all_nodes = block.inner_nodes + [block.exit_node]
            #             nid: Dict[Node, int] = {node: i for i, node in enumerate(all_nodes)}
            #             idn: Dict[int, Node] = {i: node for i, node in enumerate(all_nodes)}

            #             terms = ios.optimizer.get_input(nodes_for_merge, block, nid, idn)
            #             out_channels = sum(nd.out_channels for nd in nodes_for_merge)
            #             kernel = (max(nd.kernel[0] for nd in nodes_for_merge), max(nd.kernel[1] for nd in nodes_for_merge))
            #             stride = (nodes_for_merge[0].stride[0], nodes_for_merge[0].stride[1])
            #             padding = (max(nd.padding[0] for nd in nodes_for_merge), max(nd.padding[1] for nd in nodes_for_merge))
            #             groups = nodes_for_merge[0].groups
            #             act = nodes_for_merge[0].act
            #             conv = Conv('conv', '', inputs=terms, out_channels=out_channels, kernel=kernel, stride=stride, padding=padding, 
            #                         groups=groups, act=act, output_shape=None)
            #             merge_latency = np.mean(ios.ios_runtime.stage_latency([[conv]], batch_size, warmup, number, repeat))
            #             # origin_latency = np.mean(ios.ios_runtime.stage_latency(stage, batch_size, warmup, number, repeat))
            #             # if merge_latency < origin_latency:
            #             #     stage = [[conv]]
            #             # stage = [[conv]]
            #             block.stages[index] = [[conv]]
            #     index = index + 1
